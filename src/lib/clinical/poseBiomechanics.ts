 ...
 * Downloads ~2MB on first call; subsequent calls return cached instance.
 */
...
export async function analyzePose(
  ...
  try {
    ...
    const poseConfidence = computePoseConfidence(landmarks);

    if (poseConfidence < CLINICAL_THRESHOLDS.pose.minPoseConfidence) {
      return {
        poseConfidence,
        objectiveMetrics: {},
        objectiveFindings: [],
    ...
    const metricThresholds = (CLINICAL_THRESHOLDS.biomechanicalMetrics as any)[testKey]?.[normalizedView] as MetricThreshold[] | undefined;
    ...
    if (metricThresholds) {
      metricThresholds.forEach(threshold => {
        ...
        const flagged = threshold.direction === 'above'
          ? value > threshold.threshold
          : value < threshold.threshold;

        if (flagged) {
          objectiveFindings.push(threshold.compensationId);
        }
      });
    }
...
/**
 * Check if the pose model is available (has been loaded or can be loaded).
 */
...
/**
 * Preload the pose model (call early to reduce latency on first analysis).
 */
...
function extractMetrics(
  ...
  switch (testType) {
    case 'overhead_squat':
      extractOHSMetrics(landmarks, normalizedView, metrics);
      break;
    case 'single_leg_squat':
      extractSLSMetrics(landmarks, normalizedView, metrics);
      break;
    case 'pushup':
      extractPushupMetrics(landmarks, normalizedView, metrics);
      break;
  }
...
function extractOHSMetrics(landmarks: any[], view: string, metrics: Record<string, number>): void {
  switch (view) {
    case 'anterior': {
      // Valgus angle: angle between hip-knee-ankle in frontal plane
      const valgusL = computeAngle2D(
        landmarks[LANDMARKS.LEFT_HIP],
        landmarks[LANDMARKS.LEFT_KNEE],
        landmarks[LANDMARKS.LEFT_ANKLE],
        'xz'
      );
      const valgusR = computeAngle2D(
      ...
      // Foot orientation: angle between heel-toe in frontal plane
      ...
      metrics.footOrientation = Math.max(footL, footR) * 100; // normalize to degrees proxy
      break;
    }
    case 'lateral': {
      // Trunk lean: angle of shoulder-hip line vs vertical
      metrics.trunkLeanAngle = computeDeviationFromVertical(
        landmarks[LANDMARKS.LEFT_SHOULDER],
        landmarks[LANDMARKS.LEFT_HIP]
      );

      // Knee flexion: angle at knee joint
      metrics.kneeFlexionAngle = computeAngle2D(
        landmarks[LANDMARKS.LEFT_HIP],
        landmarks[LANDMARKS.LEFT_KNEE],
        landmarks[LANDMARKS.LEFT_ANKLE],
        'yz'
    ...
    case 'posterior': {
      // Heel rise: vertical distance between heel and foot base
      ...
      // Feet eversion: lateral deviation
...
function extractSLSMetrics(landmarks: any[], view: string, metrics: Record<string, number>): void {
  switch (view) {
    case 'anterior': {
      metrics.valgusProxy = computeAngle2D(
        landmarks[LANDMARKS.LEFT_HIP],
        landmarks[LANDMARKS.LEFT_KNEE],
        landmarks[LANDMARKS.LEFT_ANKLE],
        'xz'
      ...
      metrics.trunkLean = computeDeviationFromVertical(
        landmarks[LANDMARKS.LEFT_SHOULDER],
        landmarks[LANDMARKS.LEFT_HIP]
      );
      break;
    ...
    case 'lateral': {
      ...
      metrics.kneeFlexionProxy = computeAngle2D(
        landmarks[LANDMARKS.LEFT_HIP],
        landmarks[LANDMARKS.LEFT_KNEE],
        landmarks[LANDMARKS.LEFT_ANKLE],
        'yz'
    ...
    case 'posterior': {
      // Pelvic drop: angle between left and right hip landmarks vs horizontal
...
function extractPushupMetrics(landmarks: any[], view: string, metrics: Record<string, number>): void {
  switch (view) {
    case 'lateral': {
      // Hip sag ratio: deviation of hip from shoulder-ankle line
    ...
    case 'posterior': {
      // Elbow flare angle
      metrics.elbowFlareAngle = computeAngle2D(
        landmarks[LANDMARKS.LEFT_SHOULDER],
...
function computeDeviationFromVertical(
...
function computePoseConfidence(landmarks: any[]): number {
  if (!landmarks || landmarks.length === 0) return 0;

  const visibilityThreshold = CLINICAL_THRESHOLDS.pose.minLandmarkVisibility;
  let visibleCount = 0;

  // Check key landmarks for body analysis
  const keyIndices = [
...
function normalizeViewType(viewType: string): string {
  // Strip left_/right_ prefix for SLS views
  return viewType
    .replace('left_', '')
    .replace('right_', '');
}
